...
import {
  addDepartmentTypeApi,
  modifyDepartmentTypeApi
...
export default {
  ...
  computed: {
    isEdit() {
      return !!this.currentDepartmentType
    }
  },
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '请输入单位类型名称', trigger: 'change' },
          {
            pattern: /^.{1,16}$/,
  ...
  methods: {
    onDialogOpen() {
      // 编辑回填数据
      if (this.isEdit) {
        let { name, order, uuid } = this.currentDepartmentType
        this.form = {
          name,
          order,
          uuid
    ...
    onDialogOk() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          let api = this.isEdit ? modifyDepartmentTypeApi : addDepartmentTypeApi
          api({ ...this.form }).then(() => {
