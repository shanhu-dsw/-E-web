  ...
  <fragment>
    ...
    >
      <el-form
        ...
        label-width="auto"
      >
        <el-form-item label="上级菜单" prop="parent_uuid">
          <el-cascader
            :style="{ width: '100%' }"
            v-model="form.parent_uuid"
            :options="parentMenuList"
            :props="{ checkStrictly: true, value: 'uuid', label: 'text' }"
            placeholder="请选择上级菜单（不选默认新增一级菜单）"
            clearable
          ></el-cascader>
        </el-form-item>
...
import { addMenuApi, modifyMenuApi } from '@/api/blackPearl/securityCenter'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        text: [
          { required: true, message: '请输入显示文本', trigger: 'change' },
          {
            pattern: /^.{2,32}$/,
            message: '请输入2-32位的任意字符',
            trigger: 'change'
          }
        ],
        order: [
  ...
  methods: {
    ...
    onDialogOpen() {
      // 编辑回填数据
      if (this.isEdit) {
        let { name, text, order, description, uuid, parent_uuid } =
          this.currentMenu
    ...
    onDialogOk() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          let api = this.isEdit ? modifyMenuApi : addMenuApi
